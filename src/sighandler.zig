...
pub fn install(self: *Self) !void {
    ...
    self.handle_thread = try std.Thread.spawn(.{ .allocator = self.arena }, Self.sighandle, .{self});
    self.handle_thread.?.detach();
}

pub fn on(self: *Self, func: anytype, args: std.meta.ArgsTuple(@TypeOf(func))) !void {
...
fn sighandle(self: *Self) noreturn {
    while (true) {
        var sig: c_int = 0;

        const rc = std.c.sigwait(&self.sigset, &sig);
        if (rc != 0) {
            std.log.err("Unable to process signal: {}", .{rc});
            std.process.exit(1);
        }
        ...
        switch (sig) {
            std.posix.SIG.INT, std.posix.SIG.TERM => {
                ...
                std.log.info("Received termination signal...", .{});
